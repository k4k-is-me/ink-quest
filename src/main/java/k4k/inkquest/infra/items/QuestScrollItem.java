...
public class QuestScrollItem extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (nbt == null || !nbt.contains(NBT_QUEST_KEY, NbtElement.STRING_TYPE)) {
            user.sendMessage(Text.translatable("item.inkquest.quest_scroll.error.no_quest"), true);
            return TypedActionResult.fail(stack);
        }

        var questId = Identifier.tryParse(nbt.getString(NBT_QUEST_KEY));
        if (questId == null) {
            user.sendMessage(Text.translatable("item.inkquest.quest_scroll.error.invalid_id"), true);
            return TypedActionResult.fail(stack);
        }
        ...
        if (!manager.isQuestExists(questId)) {
            user.sendMessage(Text.translatable("item.inkquest.quest_scroll.error.unknown_quest", questId.toString()), true);
            return TypedActionResult.fail(stack);
        }

        if (!ModGameRules.canPlayerOpenQuestBook(serverPlayer)) {
            user.sendMessage(Text.translatable("item.inkquest.quest_scroll.error.no_book"), true);
            return TypedActionResult.fail(stack);
        }
        ...
        if (manager.isQuestComplete(questId, serverPlayer) && (quest == null || !quest.repeatable())) {
            user.sendMessage(Text.translatable("item.inkquest.quest_scroll.error.already_complete"), true);
            return TypedActionResult.fail(stack);
        }

        if (manager.isQuestActive(questId, serverPlayer)) {
            user.sendMessage(Text.translatable("item.inkquest.quest_scroll.error.already_active"), true);
            return TypedActionResult.fail(stack);
        }
