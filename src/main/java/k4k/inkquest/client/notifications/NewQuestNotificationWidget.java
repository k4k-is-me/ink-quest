...
import k4k.inkquest.client.animation.Animation;
import k4k.inkquest.client.animation.Animator;
import k4k.inkquest.client.animation.ParameterKey;
...
import static k4k.inkquest.client.animation.ParameterAnimations.*;
 ...
 * возвращает {@code true} и виджет удаляется из {@link k4k.inkquest.client.huds.QuestHudOverlay}.
 *
...
public class NewQuestNotificationWidget {
    ...
    private Text getMainText() {
        if (count == 1) {
            return Text.translatable("gui.inkquest.new_quest_notification.single");
        }
        return Text.translatable("gui.inkquest.new_quest_notification.multiple", count);
    }

    /** Строит подсказку клавиши (вторая строка). */
    private Text getHintText() {
        return Text.translatable(
                "gui.inkquest.new_quest_notification.hint",
